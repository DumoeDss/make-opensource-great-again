...
**Reason**: The plan route belonged to the removed local-clone implementation and exposed clone-derived target/manual-delivery concerns instead of a target-bound sealed preview.

**Migration**: None. All single and batch UI publication uses `POST /api/publish/preview`.

### Requirement: Publish stage and submit routes with a stage state model

**Reason**: Client-visible staging and in-memory per-review stage state were replaced by confirmed daemon-managed workspace/journal delivery.

**Migration**: None. The UI has no stage action and submits only the sealed publication binding to `POST /api/publish/submit`.

### Requirement: Publish preflight route

**Reason**: Five client-combined local capability booleans admit contradictory states and expose daemon implementation concerns.

**Migration**: None. The UI consumes `GET /api/publish` discriminated status.

### Requirement: Data-repo path is trusted server-side config

**Reason**: A local repository path is not the publication product object and the `--data-repo` surface has been deleted.

**Migration**: None. Settings configures one canonical GitHub `owner/repo` through `PUT /api/publish/target`.
