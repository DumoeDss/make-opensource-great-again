...
daemon 只绑定 `127.0.0.1`、无鉴权（v0.x 威胁模型：单机单用户），自托管界面在 `http://127.0.0.1:8899/ui/`。四步旅程：①选择会话 → ②处置命中 → ③签署确认 → ④选择出口（公开数据集 GitHub PR / API 直投）。

出口①配置的是公开数据集的规范 GitHub `owner/repo`，不是本地仓库路径。启动后在同源设置界面/API 中配置目标；后端只支持 `github.com`，并使用本机 `gh auth` 登录状态。预览会绑定目标版本、默认分支提交、兼容性清单和精确文件摘要，但不会创建 fork、写文件、push 或创建 PR。用户明确确认后，daemon 才在私有托管目录写入已封存字节并提交到上游或本人已验证的 fork，最终返回真实、可重试的 PR 回执。托管路径、凭据、命令和外部原始错误不会通过 HTTP 返回；GitHub 不可用时仍可只导出已脱敏、门禁通过的文件。
