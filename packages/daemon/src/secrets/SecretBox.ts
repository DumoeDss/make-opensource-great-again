 ...
 * (ported file-level from omnicross's `secrets/SecretBox.ts`, behavior verbatim,
 * error message constant renamed for mosga).
 *
 ...
 *                  later by the caller). Passthrough both ways.
 ...
 *
 * LAZY KEY: the constructor accepts either a resolved 32-byte Buffer OR a
 * `() => Buffer` resolver. The key is only resolved on the FIRST
 * `encrypt`/`decrypt` (then cached). The tri-state passthroughs (`$ENV` / empty
 * / non-envelope-on-read) return BEFORE the key is touched — so a pure
 * legacy-plaintext load that only passes values through NEVER triggers key
 * resolution, and therefore never auto-generates a keyfile. A keyfile is
 * materialized only when an `enc:` value must be decrypted or a write must
 * encrypt (the resolver does the lazy auto-gen).
 *
 * @module @mosga/daemon/secrets/SecretBox
 */

import { decryptValue, encryptValue, isEnvelope } from './envelope.js';
    ...
    try {
      ...
      throw new Error(
        'failed to decrypt a stored secret: the master key does not match (wrong ' +
          'MOSGA_MASTER_KEY / master.key) or the encrypted value was tampered with',
      );
    }
  }
