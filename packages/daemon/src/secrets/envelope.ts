/**
 * envelope.ts — the AES-256-GCM secret envelope codec (ported file-level from
 * omnicross's `secrets/envelope.ts`, behavior verbatim).
 *
 ...
 * never breaks a config's shape-guards — an `enc:` string still satisfies
 ...
 * plaintext), `v1` lets a future cipher/KDF change be recognized, and
 * AES-256-GCM gives authenticated encryption: a wrong key or a tampered envelope
 * fails the auth-tag verification at decrypt time (a clear "wrong-key" UX rather
 * than silent garbage). Each encrypt uses a FRESH random 12-byte IV (never
 * reused) + the 16-byte GCM tag.
 *
 ...
 *
 * @module @mosga/daemon/secrets/envelope
 */
 ...
 * actionable, secret-free "wrong-key or tampered" message). This raw fn
