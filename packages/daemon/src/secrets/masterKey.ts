/**
 * masterKey.ts — the daemon's 32-byte master-key resolver (ported file-level
 * from omnicross's `secrets/masterKey.ts`, constants renamed for mosga).
 *
 * Resolution order (env override beats keyfile; keyfile auto-generates last):
 *  1. `MOSGA_MASTER_KEY` env (highest) — present + non-empty → decode it as
 ...
 *  2. keyfile (default `~/.mosga/master.key`, NOT a sibling of the provider-keys
 *     / config files so a copied/committed config never drags the key along) —
 *     exists → read its raw/encoded 32 bytes.
 *  3. else AUTO-GENERATE `randomBytes(32)`, write it `0600`, and use it (so an
 *     unattended boot works with no preset key). The keyfile is only
 ...
 *     (the caller decides WHEN to resolve).
 *
 * Windows note: `chmod 0600` is effectively a no-op on win32 (NTFS ACLs don't
 * map POSIX mode). We still call it (harmless), do NOT branch on platform, and
 * document the honest caveat for the user.
 *
 * No secret material (key bytes) is ever logged or placed in an error message.
 *
 * @module @mosga/daemon/secrets/masterKey
 */
...
export const MASTER_KEY_ENV = 'MOSGA_MASTER_KEY';

/** AES-256 key length. */
const KEY_BYTES = 32;

/** The default keyfile path — a USER-PRIVATE dir, deliberately NOT next to the
 *  provider-keys / config files. */
export function defaultMasterKeyPath(): string {
  return join(homedir(), '.mosga', 'master.key');
}
...
/**
 * Decode a `MOSGA_MASTER_KEY` env value to exactly 32 bytes: 64 hex chars →
...
function generateKeyFile(path: string): Buffer {
  ...
  // systems, so chmod again. No-op on win32 — never throws here.
  try {
...
/**
 * Resolve the 32-byte master key (env → keyfile → auto-generate). Returns the
 * raw key Buffer. The keyfile is only generated/written on path 3.
 */
